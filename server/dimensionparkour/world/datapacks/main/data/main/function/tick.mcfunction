...
execute as @a at @s positioned ~ 0 ~ run damage @s[distance=..15] 1000 minecraft:out_of_world
...
scoreboard players add @e[type=marker,name=console,tag=cooldown] bossCooldown 1
execute as @e[type=marker,name=console,scores={bossCooldown=200..}] run function main:open_end_portal
execute as @e[type=marker,name=console,scores={bossCooldown=200..}] run tag @s remove cooldown
scoreboard players reset @e[type=marker,name=console,scores={bossCooldown=200..}] bossCooldown

#Summon boss
execute if entity @a[tag=bossbattle,tag=!hasWon] unless entity @e[type=ender_dragon] unless entity @e[type=marker,name=console,tag=cooldown] run function main:summon_boss

#crystal 1 check 
execute as @e[type=minecraft:marker,name=console] run function main:crystals/check

#win
execute as @e[type=marker,name=console,tag=!cooldown] if entity @e[type=ender_dragon] unless entity @e[type=end_crystal,tag=end] run function main:win/main
...
execute store result bossbar minecraft:boss value run scoreboard players get @e[type=marker,name=console,limit=1] dragonHealth
...
execute as @e[type=marker,name=console,tag=endDoorTimer_init] run scoreboard players set @s endDoorTimer 0
execute as @e[type=marker,name=console,tag=endDoorTimer_init] run tag @s add endDoorTimer
execute as @e[type=marker,name=console,tag=endDoorTimer_init] run tag @s remove endDoorTimer_init
execute as @e[type=marker,name=console,tag=endDoorTimer] run function main:end_door_timer
...
execute as @a[tag=end_tp_timer] at @s run function main:end_timer

#stop ender dragon sounds for everyone not there
stopsound @a[tag=!bossbattle] * minecraft:entity.ender_dragon.death
stopsound @a[tag=!bossbattle] * minecraft:entity.ender_dragon.ambient
stopsound @a[tag=!bossbattle] * minecraft:entity.ender_dragon.growl
stopsound @a[tag=!bossbattle] * minecraft:entity.ender_dragon.hurt
